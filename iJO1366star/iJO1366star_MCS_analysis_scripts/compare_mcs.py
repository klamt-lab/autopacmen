...
for secretion_product in secretion_products:
    ...
    geckoed_mcs_lines = [x.replace("R_", "").replace(
    ...
    original_superset_mapping = _get_superset_mapping(
        original_mcs, geckoed_mcs)
    ...
    original_num_identicals = len(set([tuple(x) for x in original_mcs]).intersection(
        [tuple(x) for x in geckoed_mcs]))
    geckoed_num_identicals = len(set([tuple(x) for x in geckoed_mcs]).intersection(
    ...
    for mcs in original_mcs:
        ...
        for reaction in mcs:
            ...
            j += 1
        worksheet.write(i, current_start + max_mcs,
                        original_subset_mapping[tuple(mcs)])
        worksheet.write(i, current_start + max_mcs + 1,
                        original_superset_mapping[tuple(mcs)])
    ...
    for current_length in range(1, max_mcs+1):
        worksheet.write(current_length, current_start +
                        max_mcs + 2, current_length)
        worksheet.write(current_length, current_start + max_mcs + 3,
    ...
    worksheet.write(max_mcs+4, current_start +
                    max_mcs + 3, original_num_identicals)
    worksheet.write(max_mcs+5, current_start + max_mcs + 2, "Neither of any")
    worksheet.write(max_mcs+5, current_start + max_mcs + 3,
    ...
    for mcs in geckoed_mcs:
        ...
        for reaction in mcs:
            ...
            j += 1
        worksheet.write(i, current_start + max_mcs,
                        geckoed_subset_mapping[tuple(mcs)])
        worksheet.write(i, current_start + max_mcs + 1,
                        geckoed_superset_mapping[tuple(mcs)])
    ...
    worksheet.write(max_mcs+4, current_start + max_mcs + 2, "Num identicals")
    worksheet.write(max_mcs+4, current_start +
                    max_mcs + 3, geckoed_num_identicals)
    worksheet.write(max_mcs+5, current_start + max_mcs + 2, "Neither of any")
    worksheet.write(max_mcs+5, current_start + max_mcs + 3,
    ...
    worksheet.write(max_mcs+1, current_start + max_mcs + 2, "SUM")
    worksheet.write(max_mcs+1, current_start + max_mcs +
                    3, len(mcs_only_in_original))
    ...
    worksheet.write(max_mcs+1, current_start + max_mcs + 2, "SUM")
    worksheet.write(max_mcs+1, current_start +
                    max_mcs + 3, len(mcs_only_in_geckoed))
