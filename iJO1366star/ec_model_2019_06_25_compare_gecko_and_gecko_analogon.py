...
for reaction in gecko.reactions:
    if "for gene rule" in reaction.name:
        continue
    metabolite_ids = [
        x.id for x in reaction.metabolites.keys() if x.id.startswith("im_")
    ]
...
for reaction in analogon.reactions:
    if "Arm reaction" in reaction.name:
        continue
    metabolite_ids = [
        x.id for x in reaction.metabolites.keys() if x.id.startswith("armm_")
    ]
    if len(metabolite_ids) == 1:
...
print(
    "===STRUCTURAL COMPARISON OF ORIGINAL GECKO AND SMOMENT-BASED GECKO-ANALOGOUS MODEL==="
)
print("Number of arm reactions - original GECKO:", len(gecko_reactions_with_arm))
print(
    "Number of arm reactions - sMOMENT GECKO analogon:",
