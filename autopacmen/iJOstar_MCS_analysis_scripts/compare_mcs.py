...
for secretion_product in secretion_products:
    ...
    geckoed_mcs_lines = [x.replace("R_", "").replace("_TG_", "TG").replace("\n", "") for x in geckoed_mcs]
    ...
    original_num_identicals = len(set([tuple(x) for x in original_mcs]).intersection([tuple(x) for x in geckoed_mcs]))
    geckoed_num_identicals = len(set([tuple(x) for x in geckoed_mcs]).intersection([tuple(x) for x in original_mcs]))
    ...
    for mcs in original_mcs:
        ...
        worksheet.write(i, current_start + max_mcs + 1, original_superset_mapping[tuple(mcs)])
    ...
    for current_length in range(1, max_mcs+1):
        ...
        worksheet.write(current_length, current_start + max_mcs + 3, len([x for x in original_mcs if len(x) == (current_length)]))
    ...
    worksheet.write(max_mcs+5, current_start + max_mcs + 3, len(original_mcs)-num_subsets-num_supersets-original_num_identicals)
    ...
    for mcs in geckoed_mcs:
        ...
        worksheet.write(i, current_start + max_mcs + 1, geckoed_superset_mapping[tuple(mcs)])
    ...
    for current_length in range(1, max_mcs+1):
        worksheet.write(current_length, current_start + max_mcs + 2, current_length)
        worksheet.write(current_length, current_start + max_mcs + 3, len([x for x in geckoed_mcs if len(x) == (current_length)]))
    worksheet.write(max_mcs+1, current_start + max_mcs + 2, "SUM")
    ...
    worksheet.write(max_mcs+5, current_start + max_mcs + 2, "Neither of any")
    worksheet.write(max_mcs+5, current_start + max_mcs + 3, len(geckoed_mcs)-num_subsets-num_supersets-geckoed_num_identicals)
    ...
    for current_length in range(1, max_mcs+1):
        worksheet.write(current_length, current_start + max_mcs + 2, current_length)
        worksheet.write(current_length, current_start + max_mcs + 3, len([x for x in mcs_only_in_original if len(x) == (current_length)]))
    worksheet.write(max_mcs+1, current_start + max_mcs + 2, "SUM")
    ...
    for current_length in range(1, max_mcs+1):
        worksheet.write(current_length, current_start + max_mcs + 2, current_length)
        worksheet.write(current_length, current_start + max_mcs + 3, len([x for x in mcs_only_in_geckoed if len(x) == (current_length)]))
    worksheet.write(max_mcs+1, current_start + max_mcs + 2, "SUM")
