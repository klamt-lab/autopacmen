...
function [f] = p_get_objective_value_for_kcats(x)
    ...
    % Change kcats
    kcats = x(1:end);
    metabolite_index = strmatch('M_prot_pool', changed_model.specID, 'exact');
    for i = 1:numel(g_reactions_to_change)
        reaction_name = g_reactions_to_change(i);
        reaction_name = reaction_name{1};
        reaction_index = strmatch(reaction_name, changed_model.reacID, 'exact');
        new_kcat = kcats(i);
        changed_model.stoichMat(metabolite_index, reaction_index) = new_kcat;
    end
